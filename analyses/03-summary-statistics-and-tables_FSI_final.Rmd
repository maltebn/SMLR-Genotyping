...
NOTE:
- To make fair and conservative comparisons of call rates, we want SMLR to have at least the same accuracy as HSG, i.e., the same number of WCs or fewer (sometimes WCs come in multiples for the same q-value, so we may not always be able to hit the same accuracies, but then we will hit an even higher accuracy, which implies having turned more observations into NCs, which favours the HSG in the call rate comparison).
- To make fair and conservative comparisons of accuracies, we want SMLR to have at least the same call rate as HSG, i.e., the same number of NCs or fewer (sometimes NCs come in multiples for the same q-value, so we may not always be able to hit the same call rates, but then we will hit an even higher call rate, which implies having turned fewer observations into NCs, which again implies a lower accuracy, thus favouring the HSG in the comparison of accuracy).
```{r Table call rate, eval=FALSE, include=FALSE}
dd_smlr_cr <- dd_pred |> 
  lapply(function(d) {
    ...
    data.frame("Low_high" = ("a" %in% d$Run)*1,
               "Tot_calls" = nrow(d),
               "HSG_accuracy" = 100*round((nrow(d)-hsg_nc-hsg_wc)/(nrow(d)-hsg_nc), digits=4),
               "SMLR_accuracy" = 100*round((nrow(d)-smlr_nc-smlr_wc)/(nrow(d)-smlr_nc), digits=4),
               "q_same_WC" = round(q_same_wc, digits=4),
               "HSG_call_rate" = 100*round((nrow(d)-hsg_nc)/nrow(d), digits=4),
               "SMLR_call_rate" = 100*round((nrow(d)-smlr_nc)/nrow(d), digits=4),
               "HSG_WC" = hsg_wc,
               "SMLR_WC" = smlr_wc,
               "HSG_NC" = hsg_nc,
               "SMLR_NC" = smlr_nc,
               "Improve_NC" = 100*round((hsg_nc-smlr_nc)/hsg_nc, digits = 4))
  }) |> bind_rows(.id = "Dilution") |> 
  mutate(Dilution = as.numeric(Dilution)) |> 
  arrange(Low_high, desc(Dilution)) |> select(-Low_high)

rownames(dd_smlr_cr) <- NULL
dd_smlr_cr
cr.table <- t(dd_smlr_cr) |> as.data.frame() |> rownames_to_column("V0")
colnames(cr.table) <- c("DNA", paste0(unlist(cr.table[1,-1]), "pg"))
cr.table <- cr.table |> filter(DNA != "Dilution")
cr.table$DNA <- c("Number of SNPs", "HSG accuracy (%)", "SMLR accuracy (%)", "q-threshold", "HSG call rate (%)", "SMLR call rate (%)", "HSG wrong calls", "SMLR wrong calls", "HSG no-calls", "SMLR no-calls", "Improvement in no-calls (%)")
cr.table <- column_to_rownames(cr.table, "DNA")
# Digits to deviate from common pattern:
dev_mat <- cr.table
dev_mat[,] <- F # Initialize to FALSE
dev_mat[4,] <- cr.table[4,] == 0 # q-thresholds to have zero digits
dev_mat[c(2:3, 5:6, 11),] <- cr.table[c(2:3, 5:6, 11),] == 100 # Accuracies, call rates, and improvements to have zero digits
dev_mat <- cbind(F, as.matrix(dev_mat))
# Digit matrix
d_mat <- matrix(c(rep(0, nrow(cr.table)), rep(c(0,2,2,4,2,2,0,0,0,0,2), ncol(cr.table))), nrow = nrow(cr.table), ncol = ncol(cr.table)+1)
d_mat[dev_mat] <- 0

cr.table <- xtable(cr.table)
digits(cr.table) <- d_mat
cr.table
```

```{r Table accuracy, eval=FALSE, include=FALSE}
dd_smlr_ac <- dd_pred |> 
  lapply(function(d) {
    hsg_wc <- dd_hsg |> ungroup() |> filter(Dilution == unique(d$Dilution)) |> select(WCs) |> unlist()
    hsg_nc <- dd_hsg |> ungroup() |> filter(Dilution == unique(d$Dilution)) |> select(NCs) |> unlist()
    ...
    data.frame("Low_high" = ("a" %in% d$Run)*1,
               ...
               "HSG_call_rate" = 100*round((nrow(d)-hsg_nc)/nrow(d), digits=4),
               "SMLR_call_rate" = 100*round((nrow(d)-smlr_nc)/nrow(d), digits=4),
               "q_same_NC" = round(q_same_nc, digits=4),
               "HSG_accuracy" = 100*round((nrow(d)-hsg_nc-hsg_wc)/(nrow(d)-hsg_nc), digits=4),
               "SMLR_accuracy" = 100*round((nrow(d)-smlr_nc-smlr_wc)/(nrow(d)-smlr_nc), digits=4),
               "HSG_NC" = hsg_nc,
               "SMLR_NC" = smlr_nc,
               "HSG_WC" = hsg_wc,
               "SMLR_WC" = smlr_wc,
               "Improve_WC" = 100*round((hsg_wc-smlr_wc)/hsg_wc, digits=4))
  }) |> bind_rows(.id = "Dilution") |> 
  mutate(Dilution = as.numeric(Dilution)) |> 
  arrange(Low_high, desc(Dilution)) |> select(-Low_high)

rownames(dd_smlr_ac) <- NULL
dd_smlr_ac

ac.table <- t(dd_smlr_ac) |> as.data.frame() |> rownames_to_column("V0")
colnames(ac.table) <- c("DNA", paste0(unlist(ac.table[1,-1]), "pg"))
ac.table <- ac.table |> filter(DNA != "Dilution")
ac.table$DNA <- c("Number of SNPs", "HSG call rate (%)", "SMLR call rate (%)", "q-threshold", "HSG accuracy (%)", "SMLR accuracy (%)", "HSG no-calls", "SMLR no-calls", "HSG wrong calls", "SMLR wrong calls", "Improvement in wrong calls (%)")
ac.table <- column_to_rownames(ac.table, "DNA")
# Digits to deviate from common pattern:
dev_mat <- ac.table
dev_mat[,] <- F # Initialize to FALSE
dev_mat[4,] <- ac.table[4,] == 0 # q-thresholds to have zero digits
dev_mat[c(2:3, 5:6, 11),] <- ac.table[c(2:3, 5:6, 11),] == 100 # Accuracies, call rates, and improvements to have zero digits
dev_mat <- cbind(F, as.matrix(dev_mat))
# Digit matrix
d_mat <- matrix(c(rep(0, nrow(ac.table)), rep(c(0,2,2,4,2,2,0,0,0,0,2), ncol(ac.table))), nrow = nrow(ac.table), ncol = ncol(ac.table)+1)
d_mat[dev_mat] <- 0

ac.table <- xtable(ac.table)
digits(ac.table) <- d_mat
ac.table
```

```{r Table call rate and accuracy, include=FALSE}
dd_smlr_cr <- dd_pred |> 
  lapply(function(d) {
    hsg_wc <- dd_hsg |> ungroup() |> filter(Dilution == unique(d$Dilution)) |> select(WCs) |> unlist()
    hsg_nc <- dd_hsg |> ungroup() |> filter(Dilution == unique(d$Dilution)) |> select(NCs) |> unlist()
    ...
    data.frame("Low_high" = ("a" %in% d$Run)*1,
               ...
               "HSG_NC" = hsg_nc,
               "HSG_WC" = hsg_wc,
               "HSG_call_rate" = 100*(nrow(d)-hsg_nc)/nrow(d),
               "HSG_accuracy" = 100*(nrow(d)-hsg_nc-hsg_wc)/(nrow(d)-hsg_nc),
               "q_same_WC" = round(q_same_wc, digits=4),
               "SMLR_aligned_WC" = smlr_wc,
               "SMLR_aligned_accuracy" = 100*(nrow(d)-smlr_nc-smlr_wc)/(nrow(d)-smlr_nc),
               "SMLR_call_rate" = 100*(nrow(d)-smlr_nc)/nrow(d),
               "SMLR_NC" = smlr_nc,
               "Improve_NC" = 100*(hsg_nc-smlr_nc)/hsg_nc)
  }) |> bind_rows(.id = "Dilution") |> 
  mutate(Dilution = as.numeric(Dilution)) |> 
  arrange(Low_high, desc(Dilution)) |> select(-Low_high)
rownames(dd_smlr_cr) <- NULL
...
dd_smlr_ac <- dd_pred |> 
  lapply(function(d) {
    ...
    data.frame("Low_high" = ("a" %in% d$Run)*1,
               ...
               "HSG_accuracy" = 100*(nrow(d)-hsg_nc-hsg_wc)/(nrow(d)-hsg_nc),
               "q_same_NC" = round(q_same_nc, digits=4),
               "SMLR_aligned_NC" = smlr_nc,
               "SMLR_aligned_call_rate" = 100*(nrow(d)-smlr_nc)/nrow(d),
               "SMLR_accuracy" = 100*(nrow(d)-smlr_nc-smlr_wc)/(nrow(d)-smlr_nc),
               "SMLR_WC" = smlr_wc,
               "Improve_WC" = 100*(hsg_wc-smlr_wc)/hsg_wc)
  }) |> bind_rows(.id = "Dilution") |> 
  mutate(Dilution = as.numeric(Dilution)) |> 
  arrange(Low_high, desc(Dilution)) |> select(-Low_high)
rownames(dd_smlr_ac) <- NULL

dd_smlr_cr_ac <- cbind(dd_smlr_cr, dd_smlr_ac[,7:12])



crac.table <- t(dd_smlr_cr_ac) |> as.data.frame() |> rownames_to_column("V0")
colnames(crac.table) <- c("DNA", paste0(unlist(crac.table[1,-1]), "pg"))
crac.table <- crac.table |> filter(DNA != "Dilution")
crac.table$DNA <- c("Number of SNPs", "HSG no-calls", "HSG wrong calls", "HSG call rate (%)", "HSG accuracy (%)", "q aligning wrong calls", "SMLR aligned wrong calls", "SMLR aligned accuracy (%)", "SMLR call rate (%)", "SMLR no-calls", "Improvement in no-calls (%)", "q aligning no-calls", "SMLR aligned no-calls", "SMLR aligned call rate (%)", "SMLR accuracy (%)", "SMLR wrong calls", "Improvement in wrong calls (%)")
crac.table <- column_to_rownames(crac.table, "DNA")
# Digits to deviate from common pattern:
dev_mat <- crac.table
dev_mat[,] <- F # Initialize to FALSE
dev_mat[c(6,12),] <- crac.table[c(6,12),] == 0 # q-thresholds to have zero digits
dev_mat[c(4:5, 8:9, 11, 14:15, 17),] <- crac.table[c(4:5, 8:9, 11, 14:15, 17),] == 100 # Accuracies, call rates, and improvements to have zero digits
dev_mat <- cbind(F, as.matrix(dev_mat))
# Digit matrix
d_mat <- matrix(c(rep(0, nrow(crac.table)), rep(c(0,0,0,2,3,4,0,3,1,0,1,4,0,1,3,0,1), ncol(crac.table))), nrow = nrow(crac.table), ncol = ncol(crac.table)+1)
d_mat[dev_mat] <- 0

crac.table <- xtable(crac.table)
digits(crac.table) <- d_mat
crac.table
```
...
colnames(ncwc.table) <- c("DNA", paste0(unlist(ncwc.table[1,-1]), "pg"))
ncwc.table <- ncwc.table |> filter(DNA != "Dilution")
ncwc.table$DNA <- c("Number of SNPs", "HSG no-calls", "HSG wrong calls", "q aligning wrong calls", "SMLR call rate", "SMLR no-calls", "Improvement in no-calls (%)", "q aligning no-calls", "SMLR accuracy", "SMLR wrong calls", "Improvement in wrong calls (%)")
ncwc.table <- column_to_rownames(ncwc.table, "DNA")
# Digits to deviate from common pattern:
dev_mat <- ncwc.table
dev_mat[,] <- F # Initialize to FALSE
dev_mat[c(4,8),] <- ncwc.table[c(4,8),] == 0 # q-thresholds to have zero digits
dev_mat[c(5,7,9,11),] <- ncwc.table[c(5,7,9,11),] == 100 # Accuracies, call rates, and improvements to have zero digits
dev_mat <- cbind(F, as.matrix(dev_mat))
# Digit matrix
d_mat <- matrix(c(rep(0, nrow(ncwc.table)), rep(c(0,0,0,4,1,0,1,4,3,0,1), ncol(ncwc.table))), nrow = nrow(ncwc.table), ncol = ncol(ncwc.table)+1)
d_mat[dev_mat] <- 0

ncwc.table <- xtable(ncwc.table)
digits(ncwc.table) <- d_mat
ncwc.table
```
...
```{r}
dd_smlr_cr_ac |> 
  mutate("CR_improvement" = SMLR_call_rate - HSG_call_rate,
         "AC_improvement" = SMLR_accuracy - HSG_accuracy) |> 
  select(Dilution, Tot_calls, HSG_call_rate, SMLR_call_rate, CR_improvement, HSG_accuracy, SMLR_accuracy, AC_improvement)
```
...
ggsave(filename = file.path("..", "article-FSIGEN", "FIG_S3-1.5-column.bmp"),
       plot = pm2, device = "bmp", width = 146.30, height = 117, units = "mm", dpi = 1000)
knitr::plot_crop(file.path("..", "article-FSIGEN", "FIG_S3-1.5-column.bmp"))
```
