---
title: "Estimate model by Mostad, Tillmar, and Kling (2023)"
...
```{r}
smlr_fit <- dd_GT_both |> filter(Dilution %in% c(50, 25)) |> fit_symmetry_model2(f = sqrt, intercept = T, b_int = c(0, 1,-2))
...
dd_GT_both_pred <- names(dd_pred_dils) |> lapply(function(x) {
  ...
  e <- 0.02
  dd_pred_dils[[x]] |>
    # predict_MTK_model(m, e, g_prior = "uniform_bi")
    # predict_MTK_model(m, e, g_prior = "uniform_tetra")
    predict_MTK_model(m, e, g_prior = genotype_freq)
}) |> bind_rows()

dd_GT_both_pred |> filter(Dilution >= 31.25) |> 
  summarise(sum(Genotype_pred != Genotype_true)) |> unlist() |> unname()
dd_GT_both_pred |> filter(Dilution >= 31.25) |> 
  summarise(sum(MTK_pred != Genotype_true)) |> unlist() |> unname()
dd_GT_both_pred |> filter(Dilution >= 31.25) |> 
...
```{r}
dd_MTK_final <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      m <- round(2*as.numeric(dil)/6)
      predict_MTK_model(x, m, e=0.06, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows()

dd_MTK_bi_final <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      m <- round(2*as.numeric(dil)/6)
      predict_MTK_model(x, m, e=0.06, "uniform_bi")
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, Run, ID, MTK_pred_u = MTK_pred, MTK_pmax_u = MTK_pmax)

dd_MTK_final <- dd_MTK_final |> 
  left_join(dd_MTK_bi_final, by = c("Target.ID", "Dilution", "Run", "ID"))


saveRDS(dd_MTK_final, file.path("..", "data", "00_dd_MTK.rds"))
# dd_MTK_final <- readRDS(file.path("..", "data", "00_dd_MTK.rds"))
```


```{r}
dd_MTK_final |> group_by(Dilution) |> 
  summarise(WC_hsg = sum(Genotype_true != Genotype & Genotype != no_call),
            WC_smlr = sum(Genotype_true != Genotype_pred),
            WC_MTK = sum(Genotype_true != MTK_pred),
            WC_bi = sum(Genotype_true != MTK_pred_u))
```



# Exploration of MTK model (led to the above choices)
The chunks below constitutes the exploration of the MTK model that was done before deciding on the final model above.
...
Lm6 <- data.frame("Dilution"=6.25, "m"=2*(2:10), "L"=sapply(2*(2:10), function(y) {predict_MTK_model(dd_pred_dils[["6.25"]], y, mle_e[["6.25"]], g_prior = genotype_freq, negloglik_me = T)}))
Lm12 <- data.frame("Dilution"=12.5, "m"=2*(2:10), "L"=sapply(2*(2:10), function(y) {predict_MTK_model(dd_pred_dils[["12.5"]], y, mle_e[["12.5"]], g_prior = genotype_freq, negloglik_me = T)}))
Lm25 <- data.frame("Dilution"=25, "m"=2*(3:12), "L"=sapply(2*(3:12), function(y) {predict_MTK_model(dd_pred_dils[["25"]], y, mle_e[["25"]], g_prior = genotype_freq, negloglik_me = T)}))
Lm31 <- data.frame("Dilution"=31.25, "m"=2*(5:10), "L"=sapply(2*(5:10), function(y) {predict_MTK_model(dd_pred_dils[["31.25"]], y, mle_e[["31.25"]], g_prior = genotype_freq, negloglik_me = T)}))
Lm50 <- data.frame("Dilution"=50, "m"=2*(6:12), "L"=sapply(2*(6:12), function(y) {predict_MTK_model(dd_pred_dils[["50"]], y, mle_e[["50"]], g_prior = genotype_freq, negloglik_me = T)}))
Lm62 <- data.frame("Dilution"=62.5, "m"=2*(7:15), "L"=sapply(2*(7:15), function(y) {predict_MTK_model(dd_pred_dils[["62.5"]], y, mle_e[["62.5"]], g_prior = genotype_freq, negloglik_me = T)}))
Lm125 <- data.frame("Dilution"=125, "m"=2*(11:20), "L"=sapply(2*(11:20), function(y) {predict_MTK_model(dd_pred_dils[["125"]], y, mle_e[["125"]], g_prior = genotype_freq, negloglik_me = T)}))
Lm250 <- data.frame("Dilution"=250, "m"=2*(21:30), "L"=sapply(2*(21:30), function(y) {predict_MTK_model(dd_pred_dils[["250"]], y, mle_e[["250"]], g_prior = genotype_freq, negloglik_me = T)}))
Lm500 <- data.frame("Dilution"=500, "m"=2*(46:56), "L"=sapply(2*(46:56), function(y) {predict_MTK_model(dd_pred_dils[["500"]], y, mle_e[["500"]], g_prior = genotype_freq, negloglik_me = T)}))
Lm1000 <- data.frame("Dilution"=1000, "m"=2*(58:67), "L"=sapply(2*(58:67), function(y) {predict_MTK_model(dd_pred_dils[["1000"]], y, mle_e[["1000"]], g_prior = genotype_freq, negloglik_me = T)}))
...
However, this isn't expected to alter the MTK model much, since $m$ enters the model as a parameter determining the proportions $q$ and $1-q$ of the two nucleotides of the true genotype: $q = k/m \sim \text{Binomial}(m, 1/2)$.
...
Le6 <- data.frame("Dilution"=6.25, "m"=mle_m[["6.25"]], "e"=e_seq, "L"=sapply(e_seq, function(y) {
  predict_MTK_model(dd_pred_dils[["6.25"]], m=mle_m[["6.25"]], e=y, g_prior = genotype_freq, negloglik_me = T)}))
Le12 <- data.frame("Dilution"=12.5, "m"=mle_m[["12.5"]], "e"=e_seq, "L"=sapply(e_seq, function(y) {
  predict_MTK_model(dd_pred_dils[["12.5"]], m=mle_m[["12.5"]], e=y, g_prior = genotype_freq, negloglik_me = T)}))
Le25 <- data.frame("Dilution"=25, "m"=mle_m[["25"]], "e"=e_seq, "L"=sapply(e_seq, function(y) {
  predict_MTK_model(dd_pred_dils[["25"]], m=mle_m[["25"]], e=y, g_prior = genotype_freq, negloglik_me = T)}))
Le31 <- data.frame("Dilution"=31.25, "m"=mle_m[["31.25"]], "e"=e_seq, "L"=sapply(e_seq, function(y) {
  predict_MTK_model(dd_pred_dils[["31.25"]], m=mle_m[["31.25"]], e=y, g_prior = genotype_freq, negloglik_me = T)}))
Le50 <- data.frame("Dilution"=50, "m"=mle_m[["50"]], "e"=e_seq, "L"=sapply(e_seq, function(y) {
  predict_MTK_model(dd_pred_dils[["50"]], m=mle_m[["50"]], e=y, g_prior = genotype_freq, negloglik_me = T)}))
Le62 <- data.frame("Dilution"=62.5, "m"=mle_m[["62.5"]], "e"=e_seq, "L"=sapply(e_seq, function(y) {
  predict_MTK_model(dd_pred_dils[["62.5"]], m=mle_m[["62.5"]], e=y, g_prior = genotype_freq, negloglik_me = T)}))
Le125 <- data.frame("Dilution"=125, "m"=mle_m[["125"]], "e"=e_seq, "L"=sapply(e_seq, function(y) {
  predict_MTK_model(dd_pred_dils[["125"]], m=mle_m[["125"]], e=y, g_prior = genotype_freq, negloglik_me = T)}))
Le250 <- data.frame("Dilution"=250, "m"=mle_m[["250"]], "e"=e_seq, "L"=sapply(e_seq, function(y) {
  predict_MTK_model(dd_pred_dils[["250"]], m=mle_m[["250"]], e=y, g_prior = genotype_freq, negloglik_me = T)}))
Le500 <- data.frame("Dilution"=500, "m"=mle_m[["500"]], "e"=e_seq, "L"=sapply(e_seq, function(y) {
  predict_MTK_model(dd_pred_dils[["500"]], m=mle_m[["500"]], e=y, g_prior = genotype_freq, negloglik_me = T)}))
Le1000 <- data.frame("Dilution"=1000, "m"=mle_m[["1000"]], "e"=e_seq, "L"=sapply(e_seq, function(y) {
  predict_MTK_model(dd_pred_dils[["1000"]], m=mle_m[["1000"]], e=y, g_prior = genotype_freq, negloglik_me = T)}))
...
When the MTK model is applied in practice, then $m$ should in principle be given for each new sample, individually.
...
m_dil_ind <- lapply(names(dd_ind), function(x) {
  ...
  m_likelihoods <- lapply(names(dd_dil), function(id) {
    sapply(m_search, function(m) {
      predict_MTK_model(dd_dil[[id]], m, e, g_prior = genotype_freq, negloglik_me = T)
    })
  })
...
m_dil_ind_bi <- lapply(names(dd_ind), function(x) {
  ...
  m_likelihoods <- lapply(names(dd_dil), function(id) {
    sapply(m_search, function(m) {
      predict_MTK_model(dd_dil[[id]], m, e, g_prior = "uniform_bi", negloglik_me = T)
    })
  })
  m_est <- sapply(m_likelihoods, function(i) {
  # for (i in m_likelihoods) {
...
All in all, the MTK model generally seems to produce the fewest WCs by choosing $e = 0.06$ and setting $m$ to the value expected from the amount of DNA.
...
Therefore, the static MTK model is evaluated directly on all samples within each dilution (configuration of $m$), and the results are appended to Table 1.

### Assessment of the number of WCs produces by different configurations of the MTK model.
```{r}
...
dd_MTK <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      m <- filter(m_dil_ind2, Dilution == dil, ID == id)$m_est
      e <- filter(e_dil_ind2, Dilution == dil, ID == id)$e_est
      predict_MTK_model(x, m, e, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows()

dd_MTK_bi <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      m <- filter(m_dil_ind2, Dilution == dil, ID == id)$m_est
      e <- filter(e_dil_ind2, Dilution == dil, ID == id)$e_est
      predict_MTK_model(x, m, e, "uniform_bi")
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, ID, MTK_pred_bi = MTK_pred, MTK_pmax_bi = MTK_pmax)

dd_MTK_01 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      # m <- filter(m_dil_ind_01, Dilution == dil, ID == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_MTK_model(x, m, e=0.01, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, ID, MTK_pred_01 = MTK_pred, MTK_pmax_01 = MTK_pmax)

dd_MTK_015 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      # m <- filter(m_dil_ind_015, Dilution == dil, ID == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_MTK_model(x, m, e=0.015, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, ID, MTK_pred_015 = MTK_pred, MTK_pmax_015 = MTK_pmax)

dd_MTK_018 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      # m <- filter(m_dil_ind_018, Dilution == dil, ID == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_MTK_model(x, m, e=0.018, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, ID, MTK_pred_018 = MTK_pred, MTK_pmax_018 = MTK_pmax)

dd_MTK_02 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      # m <- filter(m_dil_ind_02, Dilution == dil, ID == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_MTK_model(x, m, e=0.02, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, ID, MTK_pred_02 = MTK_pred, MTK_pmax_02 = MTK_pmax)

dd_MTK_03 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      # m <- filter(m_dil_ind_03, Dilution == dil, ID == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_MTK_model(x, m, e=0.03, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, ID, MTK_pred_03 = MTK_pred, MTK_pmax_03 = MTK_pmax)

dd_MTK_04 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      # m <- filter(m_dil_ind_04, Dilution == dil, ID == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_MTK_model(x, m, e=0.04, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, ID, MTK_pred_04 = MTK_pred, MTK_pmax_04 = MTK_pmax)

dd_MTK_05 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      # m <- filter(m_dil_ind_05, Dilution == dil, ID == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_MTK_model(x, m, e=0.05, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, ID, MTK_pred_05 = MTK_pred, MTK_pmax_05 = MTK_pmax)

dd_MTK_06 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
...
}) |> bind_rows() |> select(Target.ID, Dilution, ID, MTK_pred_06 = MTK_pred, MTK_pmax_06 = MTK_pmax)

dd_MTK_07 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      # m <- filter(m_dil_ind_07, Dilution == dil, ID == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_MTK_model(x, m, e=0.07, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, ID, MTK_pred_07 = MTK_pred, MTK_pmax_07 = MTK_pmax)

dd_MTK_10 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      # m <- filter(m_dil_ind_1, Dilution == dil, ID == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_MTK_model(x, m, e=0.1, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, ID, MTK_pred_10 = MTK_pred, MTK_pmax_10 = MTK_pmax)

dd_MTK_11 <- names(dd_pred_dils)[c(1:3, 5:6)] |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      # m <- filter(m_dil_ind_11, Dilution == dil, ID == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_MTK_model(x, m, e=0.11, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, ID, MTK_pred_11 = MTK_pred, MTK_pmax_11 = MTK_pmax)

dd_MTK_12 <- names(dd_pred_dils)[c(1:3, 5:6)] |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      # m <- filter(m_dil_ind_12, Dilution == dil, ID == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_MTK_model(x, m, e=0.12, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, ID, MTK_pred_12 = MTK_pred, MTK_pmax_12 = MTK_pmax)

dd_MTK_13 <- names(dd_pred_dils)[c(1:3, 5:6)] |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      # m <- filter(m_dil_ind_13, Dilution == dil, ID == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_MTK_model(x, m, e=0.13, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, ID, MTK_pred_13 = MTK_pred, MTK_pmax_13 = MTK_pmax)

dd_MTK_14 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      # m <- filter(m_dil_ind_14, Dilution == dil, ID == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_MTK_model(x, m, e=0.14, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, ID, MTK_pred_14 = MTK_pred, MTK_pmax_14 = MTK_pmax)

dd_MTK_15 <- "6.25" |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      # m <- filter(m_dil_ind_15, Dilution == dil, ID == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_MTK_model(x, m, e=0.15, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, ID, MTK_pred_15 = MTK_pred, MTK_pmax_15 = MTK_pmax)

dd_MTK_20 <- names(dd_pred_dils)[-c(9:10)] |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$ID) |> 
    lapply(function(x) {
      id <- unique(x$ID)
      # m <- filter(m_dil_ind_2, Dilution == dil, ID == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_MTK_model(x, m, e=0.2, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, ID, MTK_pred_20 = MTK_pred, MTK_pmax_20 = MTK_pmax)

dd_MTK_all <- dd_MTK |> 
  left_join(dd_MTK_bi, by = c("Target.ID", "Dilution", "ID")) |> 
  left_join(dd_MTK_01, by = c("Target.ID", "Dilution", "ID")) |> 
  left_join(dd_MTK_015, by = c("Target.ID", "Dilution", "ID")) |> 
  left_join(dd_MTK_018, by = c("Target.ID", "Dilution", "ID")) |> 
  left_join(dd_MTK_02, by = c("Target.ID", "Dilution", "ID")) |> 
  left_join(dd_MTK_03, by = c("Target.ID", "Dilution", "ID")) |> 
  left_join(dd_MTK_04, by = c("Target.ID", "Dilution", "ID")) |> 
  left_join(dd_MTK_05, by = c("Target.ID", "Dilution", "ID")) |> 
  left_join(dd_MTK_06, by = c("Target.ID", "Dilution", "ID")) |> 
  left_join(dd_MTK_07, by = c("Target.ID", "Dilution", "ID")) |> 
  left_join(dd_MTK_10, by = c("Target.ID", "Dilution", "ID")) |> 
  left_join(dd_MTK_11, by = c("Target.ID", "Dilution", "ID")) |> 
  left_join(dd_MTK_12, by = c("Target.ID", "Dilution", "ID")) |> 
  left_join(dd_MTK_13, by = c("Target.ID", "Dilution", "ID")) |> 
  left_join(dd_MTK_14, by = c("Target.ID", "Dilution", "ID")) |> 
  left_join(dd_MTK_15, by = c("Target.ID", "Dilution", "ID")) |> 
  left_join(dd_MTK_20, by = c("Target.ID", "Dilution", "ID"))
```
...
```{r}
dd_MTK_all |> group_by(Dilution) |> 
  summarise(WC_smlr = sum(Genotype_true != Genotype_pred),
            WC_gf = sum(Genotype_true != MTK_pred),
            WC_bi = sum(Genotype_true != MTK_pred_bi),
            # WC_01 = sum(Genotype_true != MTK_pred_01),
            # WC_015 = sum(Genotype_true != MTK_pred_015),
            # WC_018 = sum(Genotype_true != MTK_pred_018),
            WC_02 = sum(Genotype_true != MTK_pred_02),
            WC_03 = sum(Genotype_true != MTK_pred_03),
            WC_04 = sum(Genotype_true != MTK_pred_04),
            WC_05 = sum(Genotype_true != MTK_pred_05),
            WC_06 = sum(Genotype_true != MTK_pred_06),
            WC_07 = sum(Genotype_true != MTK_pred_07),
            WC_10 = sum(Genotype_true != MTK_pred_10),
            WC_11 = sum(Genotype_true != MTK_pred_11),
            WC_12 = sum(Genotype_true != MTK_pred_12),
            WC_13 = sum(Genotype_true != MTK_pred_13),
            WC_14 = sum(Genotype_true != MTK_pred_14),
            WC_15 = sum(Genotype_true != MTK_pred_15),
            WC_20 = sum(Genotype_true != MTK_pred_20))
```
...
But we note that the MTK model sometimes estimate a heterozygous genotype probability so high that it has been rounded to one even though the ratio $\frac{s_{min}}{s_{max}}$ was very low, so apparently the MTK model sees even a few reads as high evidence for an actual allele present: this is likely because of the very low $e$-estimates.
...
```{r}
MTK_wc <- dd_MTK_all |> 
  filter(MTK_pred != Genotype_true, Dilution >= 31.25) |> 
  left_join(allele_counts[,c("Target.ID", "N1", "N2")], by = "Target.ID")
MTK_wc <- dd_MTK_all |> 
  filter(MTK_pred_06 != Genotype_true, Dilution >= 31.25) |> 
  left_join(allele_counts[,c("Target.ID", "N1", "N2")], by = "Target.ID")
```