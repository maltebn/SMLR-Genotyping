...
library(tidyverse)
source(file.path("..", "analyses", "00_functions_and_global_definitions.R"))
```
...
nmatch <- rep(NA, length(examin_all_ID))
for (i in 1:length(nmatch)) {
  nmatch[i] <- sapply(examin_all_ID[-i], FUN = function(x) {all(x$Genotype == examin_all_ID[i]$Genotype)})
}

# Since 'nmatch' only contains TRUE-values, all Dilution x Run with complete profiles
# for all individuals do agree with each other on the profiles for each individual.
