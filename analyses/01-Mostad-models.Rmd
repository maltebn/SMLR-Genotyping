...
```{r Reading data}
dd_GT <- readRDS(file.path("..", "data", "00_dd_GT.rds")) |> filter(!(Run %in% c("A", "B")))
dd_GT_val <- readRDS(file.path("..", "data", "00_dd_GT_val.rds"))

dd_GT$Person <- interaction(dd_GT$Person, dd_GT$Run, drop = TRUE)

dd_GT_both <- dd_GT_val |> bind_rows(dd_GT[names(dd_GT_val)])
```
...
dd_mostad_final <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$Person) |> 
    lapply(function(x) {
      id <- unique(x$Person)
...
}) |> bind_rows() |> select(Target.ID, Dilution, Person, Mostad_pred_u = Mostad_pred, Mostad_pmax_u = Mostad_pmax)

dd_mostad_final <- dd_mostad_final |> 
  left_join(dd_mostad_bi_final, by = c("Target.ID", "Dilution", "Person"))
...
dd_pred <- dd_mostad_final |> 
  mutate(WC_l = Genotype_pred != Genotype_true & Genotype_pred != no_call, # logical indicators of WCs
         uWC_l = Mostad_pred_u != Genotype_true & Mostad_pred_u != no_call,
         mWC_l = Mostad_pred != Genotype_true & Mostad_pred != no_call) |> 
  split(~Dilution)
```
...
dd_smlr_nc <- dd_pred |> 
  lapply(function(d) {
    ...
    # Mostad (with prior derived from data)
    p_cr_m <- d |> filter(mWC_l) |> select(Mostad_pmax, mWC_l) |> arrange(desc(Mostad_pmax)) |> mutate(mWC_cum = cumsum(mWC_l))
    if (nrow(p_cr_m) > hsg_wc) {
      q_same_wc_m <- p_cr_m |> filter(mWC_cum <= hsg_wc) |> 
        filter(mWC_cum == max(mWC_cum)) |> select(Mostad_pmax) |> unlist()
      if (any(filter(p_cr_m, mWC_cum > hsg_wc)$Mostad_pmax == q_same_wc_m)) {
        q_same_wc_m <- p_cr_m |> filter(Mostad_pmax > q_same_wc_m) |> 
          filter(mWC_cum == max(mWC_cum)) |> select(Mostad_pmax) |> unlist()
      }
      mostad_m_nc <- sum(d$Mostad_pmax < q_same_wc_m)
    } else {
      q_same_wc_m <- 0
      mostad_m_nc <- 0
    }
    mostad_m_wc <- d |> filter(Mostad_pmax >= q_same_wc_m, mWC_l) |> nrow()
    
    # Mostad (with biallelic uniform prior)
    p_cr_u <- d |> filter(uWC_l) |> select(Mostad_pmax_u, uWC_l) |> arrange(desc(Mostad_pmax_u)) |> mutate(uWC_cum = cumsum(uWC_l))
    if (nrow(p_cr_u) > hsg_wc) {
      q_same_wc_u <- p_cr_u |> filter(uWC_cum <= hsg_wc) |> 
        filter(uWC_cum == max(uWC_cum)) |> select(Mostad_pmax_u) |> unlist()
      if (any(filter(p_cr_u, uWC_cum > hsg_wc)$Mostad_pmax_u == q_same_wc_u)) {
        q_same_wc_u <- p_cr_u |> filter(Mostad_pmax_u > q_same_wc_u) |> 
          filter(uWC_cum == max(uWC_cum)) |> select(Mostad_pmax_u) |> unlist()
      }
      mostad_u_nc <- sum(d$Mostad_pmax_u < q_same_wc_u)
    } else {
      q_same_wc_u <- 0
      mostad_u_nc <- 0
    }
    mostad_u_wc <- d |> filter(Mostad_pmax_u >= q_same_wc_u, uWC_l) |> nrow()
    ...
    data.frame("Low_high" = ("a" %in% d$Run)*1,
               ...
               "Mostad_m_call_rate" = 100*(nrow(d)-mostad_m_nc)/nrow(d),
               "Mostad_m_NC" = mostad_m_nc,
               "q_same_WC_u" = q_same_wc_u,
               "Mostad_u_call_rate" = 100*(nrow(d)-mostad_u_nc)/nrow(d),
               "Mostad_u_NC" = mostad_u_nc)
...
dd_smlr_wc <- dd_pred |> 
  lapply(function(d) {
    ...
    # Mostad (with prior derived from data)
    p_ac_m <- d |> arrange(Mostad_pmax) |> select(Mostad_pmax, mWC_l) |> rownames_to_column("idx") |> mutate(idx = as.integer(idx))
    q_same_nc_m <- p_ac_m |> filter(idx > hsg_nc) |>
      filter(idx == min(idx)) |> select(Mostad_pmax) |> unlist()
    mostad_m_wc <- d |> filter(Mostad_pmax >= q_same_nc_m, mWC_l) |> nrow()
    mostad_m_nc <- d |> filter(Mostad_pmax < q_same_nc_m) |> nrow()
    
    # Mostad (with biallelic uniform prior)
    p_ac_u <- d |> arrange(Mostad_pmax_u) |> select(Mostad_pmax_u, uWC_l) |> rownames_to_column("idx") |> mutate(idx = as.integer(idx))
    q_same_nc_u <- p_ac_u |> filter(idx > hsg_nc) |>
      filter(idx == min(idx)) |> select(Mostad_pmax_u) |> unlist()
    mostad_u_wc <- d |> filter(Mostad_pmax_u >= q_same_nc_u, uWC_l) |> nrow()
    mostad_u_nc <- d |> filter(Mostad_pmax_u < q_same_nc_u) |> nrow()
    ...
    data.frame("Low_high" = ("a" %in% d$Run)*1,
               ...
               "Mostad_m_accuracy" = 100*(nrow(d)-mostad_m_nc-mostad_m_wc)/(nrow(d)-mostad_m_nc),
               "Mostad_m_WC" = mostad_m_wc,
               "q_same_NC_u" = q_same_nc_u,
               "Mostad_u_accuracy" = 100*(nrow(d)-mostad_u_nc-mostad_u_wc)/(nrow(d)-mostad_u_nc),
               "Mostad_u_WC" = mostad_u_wc)
  }) |> bind_rows(.id = "Dilution") |> 
  mutate(Dilution = as.numeric(Dilution)) |> 
  arrange(Low_high, desc(Dilution)) |> select(-Low_high)
...
print(ncwc.table, file = file.path("..", "article2024_FSIGenetics", "final version (pre-print)", "xtable1.tex"),
...
tab1 <- read_lines(file.path("..", "article2024_FSIGenetics", "final version (pre-print)", "xtable1.tex"))
...
write_lines(tab1, file.path("..", "article2024_FSIGenetics", "final version (pre-print)", "table1.tex"))
file.remove(file.path("..", "article2024_FSIGenetics", "final version (pre-print)", "xtable1.tex"))
```



# Exploration of Mostad model
...
```{r}
dd_ind <- split(dd_GT_both_pred, dd_GT_both_pred$Dilution) |> lapply(function(x) {split(x, x$Person)})
```
...
m_dil_ind <- lapply(names(dd_ind), function(x) {
  ...
  data.frame(Person = names(dd_dil), Dilution = as.numeric(x), m_est = m_est)
}) |> bind_rows() |>
...
e_dil_ind <- lapply(names(dd_ind), function(x) {
  ...
  e_likelihoods <- lapply(names(dd_dil), function(id) {
    m <- filter(m_dil_ind, Dilution == as.numeric(x), Person == id)$m_est
  ...
  data.frame(Person = names(dd_dil), Dilution = x, e_est = e_est)
}) |> bind_rows() |>
  mutate(Dilution_f = factor(Dilution, levels = c(1000, 500, 250, 125, 62.5, 50, 31.25, 25, 12.5)))
...
m_dil_ind2 <- lapply(names(dd_ind), function(x) {
  ...
  m_likelihoods <- lapply(names(dd_dil), function(id) {
    e <- filter(e_dil_ind, Dilution == as.numeric(x), Person == id)$e_est
    sapply(m_search, function(m) {
      predict_mostad_model(dd_dil[[id]], m, e, g_prior = genotype_freq, negloglik_me = T)
    })
  })
...
```{r}
m_comp <- m_dil_ind[,1:3] |> left_join(m_dil_ind2[,1:3], by=c("Person", "Dilution"))
nrow(m_comp)
...
e_dil_ind2 <- lapply(names(dd_ind), function(x) {
  ...
  e_likelihoods <- lapply(names(dd_dil), function(id) {
    m <- filter(m_dil_ind2, Dilution == as.numeric(x), Person == id)$m_est
    sapply(e_search, function(e) {
      predict_mostad_model(dd_dil[[id]], m, e, g_prior = genotype_freq, negloglik_me = T)
    })
  })
...
This indicates a stable simultaneous optimum for both parameters.
```{r}
e_comp <- e_dil_ind[,1:3] |> left_join(e_dil_ind2[,1:3], by=c("Person", "Dilution"))
...
e_dil_ind_bi <- lapply(names(dd_ind), function(x) {
  ...
  dd_dil <- dd_ind[[x]]
  e_likelihoods <- lapply(names(dd_dil), function(id) {
    m <- filter(m_dil_ind_bi, Dilution == as.numeric(x), Person == id)$m_est
    sapply(e_search, function(e) {
      predict_mostad_model(dd_dil[[id]], m, e, g_prior = "uniform_bi", negloglik_me = T)
    })
  })
...
m_dil_ind2_bi <- lapply(names(dd_ind), function(x) {
  ...
  m_likelihoods <- lapply(names(dd_dil), function(id) {
    e <- filter(e_dil_ind_bi, Dilution == as.numeric(x), Person == id)$e_est
    sapply(m_search, function(m) {
      predict_mostad_model(dd_dil[[id]], m, e, g_prior = "uniform_bi", negloglik_me = T)
    })
  })
...
This indicates a stable simultaneous optimum for both parameters.
```{r}
m_comp <- m_dil_ind_bi[,1:3] |> left_join(m_dil_ind2_bi[,1:3], by=c("Person", "Dilution"))
sum(m_comp$m_est.x == m_comp$m_est.y)
m_comp[m_comp$m_est.x != m_comp$m_est.y,]
```
...
e_dil_ind2_bi <- lapply(names(dd_ind), function(x) {
  ...
  e_likelihoods <- lapply(names(dd_dil), function(id) {
    m <- filter(m_dil_ind2_bi, Dilution == as.numeric(x), Person == id)$m_est
    sapply(e_search, function(e) {
      predict_mostad_model(dd_dil[[id]], m, e, g_prior = "uniform_bi", negloglik_me = T)
    })
  })
...
This indicates a stable simultaneous optimum for both parameters.
```{r}
e_comp <- e_dil_ind_bi[,1:3] |> left_join(e_dil_ind2_bi[,1:3], by=c("Person", "Dilution"))
sum(e_comp$e_est.x == e_comp$e_est.y)
e_comp[e_comp$e_est.x != e_comp$e_est.y,]
```
...
```{r}
# The original Person-variable in the first dilution series has been
# overwritten with interaction(Person, Run), such that the same individual
# got new IDs for each run/examination.
dd_mostad <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$Person) |> 
    lapply(function(x) {
      id <- unique(x$Person)
      m <- filter(m_dil_ind2, Dilution == dil, Person == id)$m_est
      e <- filter(e_dil_ind2, Dilution == dil, Person == id)$e_est
...
}) |> bind_rows() |> select(Target.ID, Dilution, Person, Mostad_pred_bi = Mostad_pred, Mostad_pmax_bi = Mostad_pmax)

dd_mostad_01 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$Person) |> 
    lapply(function(x) {
      id <- unique(x$Person)
      # m <- filter(m_dil_ind_01, Dilution == dil, Person == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_mostad_model(x, m, e=0.01, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, Person, Mostad_pred_01 = Mostad_pred, Mostad_pmax_01 = Mostad_pmax)

dd_mostad_015 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$Person) |> 
    lapply(function(x) {
      id <- unique(x$Person)
      # m <- filter(m_dil_ind_015, Dilution == dil, Person == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_mostad_model(x, m, e=0.015, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, Person, Mostad_pred_015 = Mostad_pred, Mostad_pmax_015 = Mostad_pmax)

dd_mostad_018 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$Person) |> 
    lapply(function(x) {
      id <- unique(x$Person)
      # m <- filter(m_dil_ind_018, Dilution == dil, Person == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_mostad_model(x, m, e=0.018, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, Person, Mostad_pred_018 = Mostad_pred, Mostad_pmax_018 = Mostad_pmax)

dd_mostad_02 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$Person) |> 
    lapply(function(x) {
      id <- unique(x$Person)
      # m <- filter(m_dil_ind_02, Dilution == dil, Person == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_mostad_model(x, m, e=0.02, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, Person, Mostad_pred_02 = Mostad_pred, Mostad_pmax_02 = Mostad_pmax)

dd_mostad_03 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$Person) |> 
    lapply(function(x) {
      id <- unique(x$Person)
      # m <- filter(m_dil_ind_03, Dilution == dil, Person == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_mostad_model(x, m, e=0.03, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, Person, Mostad_pred_03 = Mostad_pred, Mostad_pmax_03 = Mostad_pmax)

dd_mostad_04 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$Person) |> 
    lapply(function(x) {
      id <- unique(x$Person)
      # m <- filter(m_dil_ind_04, Dilution == dil, Person == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_mostad_model(x, m, e=0.04, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, Person, Mostad_pred_04 = Mostad_pred, Mostad_pmax_04 = Mostad_pmax)

dd_mostad_05 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$Person) |> 
    lapply(function(x) {
      id <- unique(x$Person)
      # m <- filter(m_dil_ind_05, Dilution == dil, Person == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_mostad_model(x, m, e=0.05, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, Person, Mostad_pred_05 = Mostad_pred, Mostad_pmax_05 = Mostad_pmax)

dd_mostad_06 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$Person) |> 
    lapply(function(x) {
      id <- unique(x$Person)
      # m <- filter(m_dil_ind_06, Dilution == dil, Person == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_mostad_model(x, m, e=0.06, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, Person, Mostad_pred_06 = Mostad_pred, Mostad_pmax_06 = Mostad_pmax)

dd_mostad_07 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$Person) |> 
    lapply(function(x) {
      id <- unique(x$Person)
      # m <- filter(m_dil_ind_07, Dilution == dil, Person == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_mostad_model(x, m, e=0.07, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, Person, Mostad_pred_07 = Mostad_pred, Mostad_pmax_07 = Mostad_pmax)

dd_mostad_10 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$Person) |> 
    lapply(function(x) {
      id <- unique(x$Person)
      # m <- filter(m_dil_ind_1, Dilution == dil, Person == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_mostad_model(x, m, e=0.1, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, Person, Mostad_pred_10 = Mostad_pred, Mostad_pmax_10 = Mostad_pmax)

dd_mostad_11 <- names(dd_pred_dils)[c(1:3, 5:6)] |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$Person) |> 
    lapply(function(x) {
      id <- unique(x$Person)
      # m <- filter(m_dil_ind_11, Dilution == dil, Person == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_mostad_model(x, m, e=0.11, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, Person, Mostad_pred_11 = Mostad_pred, Mostad_pmax_11 = Mostad_pmax)

dd_mostad_12 <- names(dd_pred_dils)[c(1:3, 5:6)] |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$Person) |> 
    lapply(function(x) {
      id <- unique(x$Person)
      # m <- filter(m_dil_ind_12, Dilution == dil, Person == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_mostad_model(x, m, e=0.12, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, Person, Mostad_pred_12 = Mostad_pred, Mostad_pmax_12 = Mostad_pmax)

dd_mostad_13 <- names(dd_pred_dils)[c(1:3, 5:6)] |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$Person) |> 
    lapply(function(x) {
      id <- unique(x$Person)
      # m <- filter(m_dil_ind_13, Dilution == dil, Person == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_mostad_model(x, m, e=0.13, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, Person, Mostad_pred_13 = Mostad_pred, Mostad_pmax_13 = Mostad_pmax)

dd_mostad_14 <- names(dd_pred_dils) |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$Person) |> 
    lapply(function(x) {
      id <- unique(x$Person)
      # m <- filter(m_dil_ind_14, Dilution == dil, Person == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_mostad_model(x, m, e=0.14, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, Person, Mostad_pred_14 = Mostad_pred, Mostad_pmax_14 = Mostad_pmax)

dd_mostad_15 <- "6.25" |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$Person) |> 
    lapply(function(x) {
      id <- unique(x$Person)
      # m <- filter(m_dil_ind_15, Dilution == dil, Person == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_mostad_model(x, m, e=0.15, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, Person, Mostad_pred_15 = Mostad_pred, Mostad_pmax_15 = Mostad_pmax)

dd_mostad_20 <- names(dd_pred_dils)[-c(9:10)] |> lapply(function(dil) {
  split(dd_pred_dils[[dil]], dd_pred_dils[[dil]]$Person) |> 
    lapply(function(x) {
      id <- unique(x$Person)
      # m <- filter(m_dil_ind_2, Dilution == dil, Person == id)$m_est
      m <- round(2*as.numeric(dil)/6)
      predict_mostad_model(x, m, e=0.2, genotype_freq)
    }) |> bind_rows()
}) |> bind_rows() |> select(Target.ID, Dilution, Person, Mostad_pred_20 = Mostad_pred, Mostad_pmax_20 = Mostad_pmax)

dd_mostad_all <- dd_mostad |> 
  left_join(dd_mostad_bi, by = c("Target.ID", "Dilution", "Person")) |> 
  left_join(dd_mostad_01, by = c("Target.ID", "Dilution", "Person")) |> 
  left_join(dd_mostad_015, by = c("Target.ID", "Dilution", "Person")) |> 
  left_join(dd_mostad_018, by = c("Target.ID", "Dilution", "Person")) |> 
  left_join(dd_mostad_02, by = c("Target.ID", "Dilution", "Person")) |> 
  left_join(dd_mostad_03, by = c("Target.ID", "Dilution", "Person")) |> 
  left_join(dd_mostad_04, by = c("Target.ID", "Dilution", "Person")) |> 
  left_join(dd_mostad_05, by = c("Target.ID", "Dilution", "Person")) |> 
  left_join(dd_mostad_06, by = c("Target.ID", "Dilution", "Person")) |> 
  left_join(dd_mostad_07, by = c("Target.ID", "Dilution", "Person")) |> 
  left_join(dd_mostad_10, by = c("Target.ID", "Dilution", "Person")) |> 
  left_join(dd_mostad_11, by = c("Target.ID", "Dilution", "Person")) |> 
  left_join(dd_mostad_12, by = c("Target.ID", "Dilution", "Person")) |> 
  left_join(dd_mostad_13, by = c("Target.ID", "Dilution", "Person")) |> 
  left_join(dd_mostad_14, by = c("Target.ID", "Dilution", "Person")) |> 
  left_join(dd_mostad_15, by = c("Target.ID", "Dilution", "Person")) |> 
  left_join(dd_mostad_20, by = c("Target.ID", "Dilution", "Person"))
```
