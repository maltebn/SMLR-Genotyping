...
What is the proportion of heterozygous genotypes for each individual?
```{r}
(het_props_orig <- dd_GT |> filter(Dilution == 1000) |> 
  group_by(ID) |> 
  summarise("Proportion a1a1" = sum(Genotype_true_AA == "A1A1")/n(),
            "Proportion a2a2" = sum(Genotype_true_AA == "A2A2")/n(),
            "Proportion a1a2" = sum(Genotype_true_AA == "A1A2")/n()))

(het_props_val <- true_profiles_val |> group_by(ID) |> 
  summarise("Proportion a1a1" = sum(Genotype_true_AA == "A1A1")/n(),
            "Proportion a2a2" = sum(Genotype_true_AA == "A2A2")/n(),
            "Proportion a1a2" = sum(Genotype_true_AA == "A1A2")/n()))

het_props <- c(het_props_orig$`Proportion a1a2`, het_props_val$`Proportion a1a2`)
eb <- ebeta(het_props)
bp <- seq(0.15,0.35, 0.0125)
hist(het_props, breaks = seq(0.15,0.40, 0.0125), probability = T)
lines(bp, dbeta(bp, eb$parameters[[1]], eb$parameters[[2]]), col = 2, lwd = 2)
```
...
init_model_dil_fine <- lapply(dils, function(d) {
  ...
  m$Het_prop_pop <- nrow(filter(true_profiles_val, Genotype_true_AA == "A1A2")) / nrow(true_profiles_val)
  m$f <- "sqrt"
  m$beta0 <- m$par[1]
  m$beta1 <- m$par[2]
  m$beta2 <- m$par[3]
...
dd_boot_fine <- readRDS(file.path("..", "data", "00_obj_bootstrap_sqrt1K.rds"))
...
ggsave(filename = file.path("..", "article-FSIGEN", "FIG_S1-two-column.bmp"),
       plot = p, device = "bmp", width = 199.67, height = 116.83, units = "mm", dpi = 1000)
knitr::plot_crop(file.path("..", "article-FSIGEN", "FIG_S1-two-column.bmp"))
```
