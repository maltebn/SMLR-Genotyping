...
ggsave(filename = file.path("..", "article-FSIGEN", "FIG_4-two-column.bmp"),
       plot = p, device = "bmp", width = 197.73, height = 71.23, units = "mm", dpi = 1000)
knitr::plot_crop(file.path("..", "article-FSIGEN", "FIG_4-two-column.bmp"))
```
...
ggsave(filename = file.path("..", "article-FSIGEN", "FIG_3-two-column.bmp"),
       plot = p, device = "bmp", width = 197.72, height = 69.35, units = "mm", dpi = 1000)
knitr::plot_crop(file.path("..", "article-FSIGEN", "FIG_3-two-column.bmp"))
```
...
knitr::plot_crop(file.path("..", "article-FSIGEN", "accuracy_vs_call_rate_bins.bmp"))
```
...
SMLR_med_ac_cr_low <- dd_models$low |> filter(Model == "sqrt") |> select(CR_bin, Median_of_mean_AC, Bin_count)
SMLR_med_ac_cr_low |> filter(Median_of_mean_AC >= hsg_med_ac_cr_low$Accuracy) |> 
...
# When does SMLR surpass HSG's median accuracy and what is its accuracy at call rate 95%
SMLR_med_ac_cr_high <- dd_models$high |> filter(Model == "sqrt") |> select(CR_bin, Median_of_mean_AC, Bin_count)
SMLR_med_ac_cr_high |> filter(Median_of_mean_AC >= hsg_med_ac_cr_high$Accuracy) |> 
  filter(CR_bin == max(CR_bin))
SMLR_med_ac_cr_high |> filter(Median_of_mean_AC == 100) |> 
  filter(CR_bin == max(CR_bin))
...
# When does SMLR surpass HSG's median accuracy and what is its accuracy at call rate 95%
SMLR_med_ac_cr_very_high <- dd_models$very_high |> filter(Model == "sqrt") |> select(CR_bin, Median_of_mean_AC, Bin_count)
SMLR_med_ac_cr_very_high |> filter(Median_of_mean_AC >= hsg_med_ac_cr_very_high$Accuracy) |> 
  filter(CR_bin == max(CR_bin))
...
```{r}
dd_val_dils <- readRDS(file.path("..", "data", "01_obj_crossval_NI_1K_9individuals_high_all.rds"))
# names(dd_val_dils) <- LETTERS[1:length(dd_val_dils)]
names(dd_val_dils) <- "D"
```
