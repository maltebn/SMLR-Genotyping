...
In a recent study, Mostad et al.~\cite{mostad} present a probabilistic approach to handle missing data (no-calls) in low-coverage sequencing, where they integrate genotype likelihoods directly into computations on familial relationships.
...
Through the parameters of a multinomial distribution, their model elegantly seeks to handle both imbalances in PCR amplicons and noise reads unrelated to the true genotype, but is therefore limited to integer data.
...
Its symmetric formulation ensures it remains indifferent to the order of the allele signals, making the SMLR model robust yet straightforward in its assumptions.
...
In a recent article~\cite{mostad}, Mostad et al. define a model for the observed counts $c = (c_A, c_C, c_G, c_T)$ at each nucleotide.
...
\end{align*}
were $0 < e <1$ is an overall error parameter related to noise counts, such that each count has the following independent probability of being a type not present in the true genotype:
...
\end{align*}
If the sample to be genotyped is from an individual with known origin, it is natural to base $P(g)$ on population allele frequencies, but in forensic genetics, such knowledge is often not present.
...
For a biallelic marker with alleles $a_1$ and $a_2$ having measured signals $s_1$ and $s_2$, we consider the unphased genotype $G$ with outcomes $\{a_1a_1, a_1a_2, a_2a_2\}$ and conditional genotype probabilities $p_{ij} = P\left(G = a_i a_j \mid s1, s2 \right)$.
...
This article do not seek to evaluate the MTK model’s sensitivity to variations in the $e$ and $m$ parameters or to the specification of $P(g)$.
...
Due to limitations in the initial DNA amounts, one of the examinations included only a single individual at \SI{62.5}{\pg} DNA, and another included only two individuals at \SI{31.25}{\pg}.
...
The MLEs of $m$ matched~\eqref{eq:m} quite well for the lower DNA amounts, but had large discrepancies for the high DNA amounts.
...
Using the MLEs for $e$ gave rather low classification accuracies, since the model would interpret many noise counts as evidence for an allele due the very low $e$.
...
However, a direct parameter search showed that $e = 0.06$ would provide among the highest classification accuracies for each DNA amount, and that the accuracy wouldn’t change whether $m$ was based on the DNA quantity or maximum likelihood estimation.
Thus, in the following, the MTK model is used with $e = 0.06$ and $m$ is simply set to the quantity-based version from~\eqref{eq:m}.
...
Table~\ref{tab:NC_WC} also shows that the accuracy of the SMLR model is comparable to that of the MTK model, with a slight advantage for SMLR across all DNA amounts, and furthermore that the SMLR model has a call rate advantage, particularly at 62.5 and \SI{50}{\pg} DNA.
\subfile{table1.tex}
...
Non-symmetric decision boundaries also risk making some nucleotides more prone to be declared NCs than others, creating an NC bias.
Furthermore, even with significant differences in the nucleotide signal distributions (such that certain allele labellings make more sense than others), it is still quite likely that the SMLR model's equal treatment of alleles will yield higher accuracy compared to a non-symmetric model due to the bias-variance trade-off~\cite{shmueli}.
However, we don't see any alarming differences in the nucleotides' signals (see Fig.~\ref{fig:s_density}).


Table~\ref{tab:NC_WC} shows that the SMLR and MTK models have comparable accuracies and call rates across all examined DNA quantities, with SMLR generally surpassing MTK, especially at 50 and \SI{62.5}{\pg} DNA, where its call rate is 9\% and 12\% higher.
...
We conducted separate preliminary empirical assessments for the homozygous and heterozygous genotypes within the combined DNA amounts of 31.25 and \SI{50}{\pg}.
Through binomial testing on groups of observations, we evaluated whether the mean of the model’s probability estimates for its predicted genotype could explain the empirical proportion of that genotype.
This method confirmed that the SMLR model's probability estimates align well with empirical genotype distributions, reinforcing our confidence in its utility for forensic applications.
